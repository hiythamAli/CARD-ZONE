...
<button class="cat-add" onclick="event.stopPropagation();openCheckout(${p.id},'${p.variants[0].id}')">+</button>        </div>
      </div>
    </div>`;
  }).join('');
}
  ...
  body.innerHTML = `
    <div class="cartpage-grid">
      ...
      <div class="summary-box">
        ...
        <button class="cart-checkout" onclick="openCartCheckout()">
          إتمام الشراء الآمن 🔒
        </button>
  ...
  const cols = [
    ...
    },
    {
      title: 'تواصل معنا', items: [
        ...
        { label: '📧 hiythamali69@gmail.com', page: 'contact' },
        ...
        ${cols.map(col => `
          <div class="footer-col">
            ...
            <ul>
              ${col.items.map(item => `
                <li>
                  <a href="#" onclick="event.preventDefault();${item.page
